...
class CommentListContainer extends Component {
    ...
    _loadComments() {
        ...
        this.props.initComments(comments)  //这里调用的是mapDispatchToProps中方法
    }
    //这是对持久层数据做实际操作
    handleDeleteComment(index) {
        ...
        if (this.props.onDeleteComment) {
            ...
            this.props.onDeleteComment(index)  //这里调用的是mapDispatchToProps中方法
        }
    }
...
//mapStateToProps,mapDispatchToProps,connect是关联容器组件和state的，
//对state的操作在reducer中进行
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        initComments: (comments) => {
            dispatch(initComments(comments))    //这里调用的是reducer中方法，去进行对state的操作
        },
        // 删除评论
        onDeleteComment: (commentIndex) => {
            dispatch(deleteComment(commentIndex))   //这里调用的是reducer中方法
        }
    }
}
